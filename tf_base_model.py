...
class TFBaseModel(object):
        ...
        with self.session.as_default():
            ...
            while step < self.num_training_steps:
                ...
                if step % self.log_interval == 0:
                    ...
                    metric_log = (
                        f"[[step {step:>8}]]     "
                        f"[[train {round(avg_train_time, 4):>4}s]]     loss: {round(avg_train_loss, 8):<12}     "
                        f"[[val {round(avg_val_time, 4):>4}s]]     loss: {round(avg_val_loss, 8):<12}     "
                    )
                    ...
                    for metric_name, metric_history in metric_histories.items():
                        ...
                        metric_log += f'{metric_name}: {round(metric_val, 4):<4}     '
                    ...
                    if step - best_validation_tstep > self.early_stopping_steps:
                        ...
                        if self.num_restarts is None or self.restart_idx >= self.num_restarts:
                            logging.info(f'best validation loss of {best_validation_loss} at training step {best_validation_tstep}')
    ...
    def predict(self, chunk_size=256):
        ...
        if hasattr(self, 'prediction_tensors'):
            ...
            for tensor_name, tensor in prediction_dict.items():
                ...
                save_file = os.path.join(self.prediction_dir, f'{tensor_name}.npy')
                logging.info(f'saving {tensor_name} with shape {np_tensor.shape} to {save_file}')
    ...
    def save(self, step, averaged=False):
        ...
        if not os.path.isdir(checkpoint_dir):
            logging.info(f'creating checkpoint directory {checkpoint_dir}')
    ...
    def restore(self, step=None, averaged=False):
        ...
        if not step:
            ...
            model_path = os.path.join(
                checkpoint_dir, f"model{'_avg' if averaged else ''}-{step}"
            )
        logging.info(f'restoring model parameters from {model_path}')

        # Create variable mapping from checkpoint names to graph variables
        reader = tf.train.NewCheckpointReader(model_path)
        ckpt_vars = reader.get_variable_to_shape_map()

        def normalize_name(name):
            """Remove scope prefixes like 'while/', 'cond/', etc."""
            parts = name.split('/')
            filtered = [p for p in parts if p not in ('while', 'cond', 'cond_1', 'cond_2')]
            return '/'.join(filtered)

        # Build mapping and track variables needing special handling
        var_map = {}
        kernel_splits = []  # [(ckpt_kernel_name, model_kernel_var, model_recurrent_var)]
        lstm_biases = []  # [(ckpt_name, model_var)] - biases that need gate reordering
        unmapped_vars = []
        ...
        for var in all_vars:
            ...
            # Skip recurrent_kernel - we'll handle it with kernel
            if 'recurrent_kernel' in var_name:
                continue
            ...
            for ckpt_name in ckpt_vars:
                if 'RMSProp' in ckpt_name:
                    continue

                if normalize_name(ckpt_name) == normalized:
                    if ckpt_vars[ckpt_name] == var.shape.as_list():
                        # Check if this is an LSTM bias that needs gate reordering
                        if 'lstm_cell' in ckpt_name and '/bias' in ckpt_name:
                            lstm_biases.append((ckpt_name, var))
                        else:
                            var_map[ckpt_name] = var
                        found = True
                        break

            # Check for kernel split case (Keras separates kernel/recurrent_kernel)
            if not found and '/kernel' in var_name and 'recurrent_kernel' not in var_name:
                # Find corresponding recurrent_kernel
                recurrent_name = var_name.replace('/kernel', '/recurrent_kernel')
                recurrent_var = None
                for v in all_vars:
                    if v.name.rstrip(':0') == recurrent_name:
                        recurrent_var = v
                        break

                if recurrent_var is not None:
                    # Find checkpoint kernel
                    for ckpt_name in ckpt_vars:
                        if 'RMSProp' in ckpt_name:
                            continue
                        if normalize_name(ckpt_name) == normalized:
                            # Check if shapes are compatible for split
                            ckpt_shape = ckpt_vars[ckpt_name]
                            kernel_shape = var.shape.as_list()
                            recurrent_shape = recurrent_var.shape.as_list()
                            if (len(ckpt_shape) == 2 and len(kernel_shape) == 2 and
                                ckpt_shape[0] == kernel_shape[0] + recurrent_shape[0] and
                                ckpt_shape[1] == kernel_shape[1] == recurrent_shape[1]):
                                kernel_splits.append((ckpt_name, var, recurrent_var))
                                found = True
                                break
        ...
        logging.info(f'Mapped {len(var_map)} variables directly from checkpoint')
        if kernel_splits:
            logging.info(f'Found {len(kernel_splits)} kernels requiring split')
        if lstm_biases:
            logging.info(f'Found {len(lstm_biases)} LSTM biases requiring gate reorder')
        if unmapped_vars:
            logging.info(f'Unmapped variables: {[v for v in unmapped_vars if "RMSProp" not in v][:5]}')

        # First restore directly mapped variables
        if var_map:
            mapped_saver = tf.train.Saver(var_map)
            mapped_saver.restore(self.session, model_path)

        # IMPORTANT: TF1 LSTMCell uses gate order [i, c, f, o] (input, cell, forget, output)
        # Keras LSTMCell uses gate order [i, f, c, o] (input, forget, cell, output)
        # We need to reorder the gates when loading weights
        def reorder_lstm_gates_2d(weights):
            """Reorder 2D LSTM weights from TF1 [i,c,f,o] to Keras [i,f,c,o] order."""
            # weights shape: [input_dim, 4*hidden_size]
            i, c, f, o = np.split(weights, 4, axis=1)
            return np.concatenate([i, f, c, o], axis=1)

        def reorder_lstm_gates_1d(bias, add_forget_bias=True):
            """Reorder 1D LSTM bias from TF1 [i,c,f,o] to Keras [i,f,c,o] order.
            Also adds forget_bias (1.0) since TF1 LSTMCell adds this at runtime but Keras doesn't."""
            # bias shape: [4*hidden_size]
            i, c, f, o = np.split(bias, 4)
            # TF1 LSTMCell adds forget_bias (default 1.0) to forget gate at runtime
            # Keras doesn't do this, so we add it to the loaded bias
            if add_forget_bias:
                f = f + 1.0
            return np.concatenate([i, f, c, o])

        # Handle kernel splits with gate reordering
        for ckpt_name, kernel_var, recurrent_var in kernel_splits:
            ckpt_value = reader.get_tensor(ckpt_name)
            kernel_size = kernel_var.shape.as_list()[0]

            kernel_part = ckpt_value[:kernel_size, :]
            recurrent_part = ckpt_value[kernel_size:, :]

            # Reorder gates from TF1 to Keras order
            kernel_part = reorder_lstm_gates_2d(kernel_part)
            recurrent_part = reorder_lstm_gates_2d(recurrent_part)

            self.session.run(kernel_var.assign(kernel_part))
            self.session.run(recurrent_var.assign(recurrent_part))
            logging.info(f'Split and reordered {ckpt_name} into kernel and recurrent_kernel')

        # Handle LSTM bias reordering (with forget_bias addition)
        for ckpt_name, bias_var in lstm_biases:
            ckpt_value = reader.get_tensor(ckpt_name)
            reordered_bias = reorder_lstm_gates_1d(ckpt_value, add_forget_bias=True)
            self.session.run(bias_var.assign(reordered_bias))
            logging.info(f'Reordered bias gates and added forget_bias for {ckpt_name}')
    ...
    def update_parameters(self, loss):
        ...
        # Filter out None gradients (variables not used in loss computation)
        clipped = [(tf.clip_by_value(g, -self.grad_clip, self.grad_clip), v_) for g, v_ in grads if g is not None]
